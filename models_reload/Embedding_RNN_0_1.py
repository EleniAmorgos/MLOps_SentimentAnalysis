# -*- coding: utf-8 -*-
"""
Created on Wed Dec 13 01:37:32 2023
...
from tensorflow.keras.layers import Dense, GlobalAveragePooling1D, Input, Embedding, Dropout, LSTM, RNN, GRUCell

embedding_size = 200

model_0_1_embedding_1 = Sequential()
model_0_1_embedding_1.add(Embedding(input_dim=vocabulary_size, output_dim=embedding_size))
model_0_1_embedding_1.add(RNN(GRUCell(128), return_sequences=True))
model_0_1_embedding_1.add(Dropout(0.3))
model_0_1_embedding_1.add(GlobalAveragePooling1D())
model_0_1_embedding_1.add(Dense(256, activation='relu'))
model_0_1_embedding_1.add(Dropout(0.3))
model_0_1_embedding_1.add(Dense(1, activation='sigmoid'))
...
model_0_1_embedding_1.save('datasets/SatisfactionClients/model_0_1_embedding_rnn')

# evaluation du modele : liste avec la loss et l accuracy
evaluation_model_rnn_0_1 = model_0_1_embedding_1.evaluate(X_pad_test1, y_test1, verbose=1)
...
fig_0_1.savefig('predictions_rnn_0_1.png')

# comparaison des prédictions et des notes du jeu de test
check_df_rnn_0_1 = pd.DataFrame(list(zip(df_test_0_1.commentaire, df_test_0_1.note, check_predict_0_1_class)), columns=['commentaire', 'note', 'prediction'])

from sklearn.metrics import classification_report

cm_test1_rnn_0_1 = classification_report(df_test_0_1.note, check_predict_0_1_class)
cm_test2_rnn_0_1 = pd.crosstab(df_test_0_1.note, check_predict_0_1_class, rownames=['données réelles'], colnames=['predictions'])
cm_test3_rnn_0_1 = pd.crosstab(df_test_0_1.note, check_predict_0_1_class, rownames=['données réelles'], colnames=['predictions'], normalize=0)
