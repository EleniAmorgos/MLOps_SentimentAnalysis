# -*- coding: utf-8 -*-
"""
Created on Wed Dec 13 13:07:41 2023
...
model_embedding_1.add(Embedding(input_dim=vocabulary_size, output_dim=embedding_size))
model_embedding_1.add(RNN(GRUCell(128), return_sequences=True))
model_embedding_1.add(Dropout(0.3))
model_embedding_1.add(GlobalAveragePooling1D())
model_embedding_1.add(Dense(256, activation='relu'))
model_embedding_1.add(Dropout(0.3))
model_embedding_1.add(Dense(6, activation='softmax'))
...
model_embedding_1.save('datasets/SatisfactionClients/model_embedding_rnn')

# evaluation du modele : liste avec la loss et l accuracy
evaluation_model_rnn = model_embedding_1.evaluate(X_pad_test, y_test, verbose=1)
...
check_df_rnn = pd.DataFrame(list(zip(df_test.commentaire, df_test.note, check_predict_class)), columns=['commentaire', 'note', 'prediction'])


from sklearn.metrics import classification_report

cr_rnn = classification_report(y_test, predict_cdiscount_class)
cm_rnn = pd.crosstab(y_test, predict_cdiscount_class, rownames=['données réelles'], colnames=['predictions'])

# vérification des résultats sur un jeu de test externe (100 commentaires amazon également répartis entre les étoiles)
cm_test_rnn = classification_report(df_test.note, check_predict_class)
cm_test2_rnn = pd.crosstab(df_test.note, check_predict_class, rownames=['données réelles'], colnames=['predictions'])
cm_test3_rnn = pd.crosstab(df_test.note, check_predict_class, rownames=['données réelles'], colnames=['predictions'], normalize=0)