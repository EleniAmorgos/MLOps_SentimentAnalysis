# -*- coding: utf-8 -*-
"""
Created on Wed Dec 13 21:02:19 2023
...
model_embedding_1.save('datasets/SatisfactionClients/model_embedding_emb')
...
fig.savefig('predictions_emb.png')

# comparaison des prédictions et des notes du jeu de test
check_df_emb = pd.DataFrame(list(zip(df_test.commentaire, df_test.note, check_predict_class)), columns=['commentaire', 'note', 'prediction'])


from sklearn.metrics import classification_report

cr_emb = classification_report(y_test, predict_cdiscount_class)
cm_emb = pd.crosstab(y_test, predict_cdiscount_class, rownames=['données réelles'], colnames=['predictions'])

# vérification des résultats sur un jeu de test externe (100 commentaires amazon également répartis entre les étoiles)
cm_test_emb = classification_report(df_test.note, check_predict_class)
cm_test2_emb = pd.crosstab(df_test.note, check_predict_class, rownames=['données réelles'], colnames=['predictions'])
cm_test3_emb = pd.crosstab(df_test.note, check_predict_class, rownames=['données réelles'], colnames=['predictions'], normalize=0)